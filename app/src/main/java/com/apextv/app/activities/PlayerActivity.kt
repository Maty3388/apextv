...
import com.apextv.app.databinding.ActivityPlayerBinding
...
class PlayerActivity : AppCompatActivity() {
    ...
    private var urlIdx = 0
    private var networkMonitor: com.apextv.app.utils.NetworkMonitor? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        networkMonitor = com.apextv.app.utils.NetworkMonitor(this).apply {
            onDisconnected = {
                wasDisconnected = true
                runOnUiThread { android.widget.Toast.makeText(this@PlayerActivity, "Sin conexión", android.widget.Toast.LENGTH_SHORT).show() }
            }
            onConnected = {
                if (wasDisconnected && !isRetrying) {
                    ...
                    runOnUiThread {
                        ...
                        retries = 0; urlIdx = 0; loadChannel(idx)
                    }
                }
            }
            start()
    ...
    private fun initPlayer() {
        ...
        player?.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(state: Int) {
                when (state) {
                    Player.STATE_READY -> {
                        showLoading(false); retries = 0; urlIdx = 0
                        loadTimer?.cancel(); isRetrying = false; wasReady = true
                    }
                    Player.STATE_BUFFERING -> {
                        showLoading(true)
                        if (!wasReady && !isRetrying) startLoadTimer()
                    }
                ...
                if (urlIdx < urls.size - 1) {
                    ...
                    scope.launch {
                        val fresh = withContext(Dispatchers.IO) {
                            try {
                                com.apextv.app.services.ApiService.getChannels()
    ...
    private fun buildClearKeySessionManager(drmKeys: String): DefaultDrmSessionManager? {
        ...
        val keysJson = drmKeys.split(",").mapNotNull { pair ->
            ...
            val kid = hexToBase64Url(parts[0]); val k = hexToBase64Url(parts[1])
            """{"kty":"oct","kid":"$kid","k":"$k"}"""
        }
    ...
    private fun getUrls(ch: Channel): List<String> =
        ch.streamUrl.split("|").map { it.trim() }.filter { it.isNotEmpty() }

    private fun loadChannel(i: Int) {
        if (channels.isEmpty()) return
        loadTimer?.cancel(); wasReady = false
    ...
    private fun playUrl(url: String) {
        ...
        val src = when {
            ...
            url.contains(".mpd") -> {
                ...
                buildClearKeySessionManager(ch.drmKeys)?.let { drmMgr ->
                    dashFactory.setDrmSessionManagerProvider { drmMgr }
                }
        ...
        if (ch.id.isNotEmpty() && !ch.isLive) {
            val savedPos = com.apextv.app.utils.Prefs.getProgress(this, ch.id)
            if (savedPos > 0) {
                player?.addListener(object : Player.Listener {
                    override fun onPlaybackStateChanged(state: Int) {
    ...
    private fun updateOverlay(ch: Channel, i: Int) {
    ...
    private fun startLoadTimer() {
        ...
        loadTimer = object : CountDownTimer(15000, 15000) {
            ...
            override fun onFinish() {
                if (!isRetrying && retries < 3) {
                    isRetrying = true; retries++
                    scope.launch { delay(500); loadChannel(idx); isRetrying = false }
    ...
    override fun onDestroy() {
        ...
        try {
            ...
            if (ch != null && ch.id.isNotEmpty()) {
                com.apextv.app.utils.Prefs.saveProgress(this, ch.id, pos, dur)
            }
        } catch (_: Exception) {}
        loadTimer?.cancel(); controlsTimer?.cancel()
        scope.cancel(); player?.release(); networkMonitor?.stop()
    }
}
