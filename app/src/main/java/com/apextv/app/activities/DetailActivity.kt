...
import com.apextv.app.databinding.ActivityDetailBinding
import com.apextv.app.models.Channel
import com.apextv.app.models.Episode
...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        else @Suppress("DEPRECATION") intent.getSerializableExtra(EXTRA_MOVIE) as? Movie

        val serie = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU)
            intent.getSerializableExtra(EXTRA_SERIE, Serie::class.java)
        else @Suppress("DEPRECATION") intent.getSerializableExtra(EXTRA_SERIE) as? Serie

        if (movie == null && serie == null) { finish(); return }
...
class EpisodeAdapter(private val items: List<Episode>, private val onClick: (Episode, Int) -> Unit)
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.itemView.setOnFocusChangeListener { v, focused ->
            v.setBackgroundColor(if (focused) 0xFF16162A.toInt() else 0xFF0F0F1A.toInt())
        }
    }
